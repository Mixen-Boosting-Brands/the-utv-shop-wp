...
if (function_exists("add_theme_support")) {
    ...
    add_image_size("thumb-square", 316, 316, true); // Custom Thumbnail Size call using the_post_thumbnail('custom-size');
    add_image_size("thumb-blog", 316, 500, true); // Custom Thumbnail Size call using the_post_thumbnail('custom-size');
...
add_action("init", function () {
    if (
        !empty($_GET["filter_vehicle-model"]) &&
        empty($_GET["filter_pa_vehicle-model"])
    ) {
        $_GET["filter_pa_vehicle-model"] = sanitize_text_field(
            wp_unslash($_GET["filter_vehicle-model"]),
        );
    }
});
